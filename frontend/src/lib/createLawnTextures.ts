...
export interface LawnTexturePack {
...
/**
 * Pack de texture GAZON procédural (fonction pure, ex-corps de useLawnTexture) :
 * champ d'herbe généré par fBm — variations de couleur douces (albédo),
 * micro-relief (normal map dérivée du champ de hauteur par Sobel), et rugosité.
 * Le tout se répète sans couture (bruit torique). Sert de surface « gazon » au
 * splatting du terrain unifié ET au hook useLawnTexture.
 */
export function createLawnTextures(repeat: number): LawnTexturePack {
